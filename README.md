...
First, fixes the OpenAPI spec, then lints it (unless linting is disabled by config). Next, regenerates SDKs and creates a changeset file. By default, generated SDK changesets use a `patch` semver bump and the `Regenerate SDKs` message; reusable workflow callers can pass `changeset_type` with `patch`, `minor`, or `major` to choose a different bump level, and `changeset_message` to customize the generated changeset message for manual regeneration flows. The selected type is passed to `konfig changeset` through the CLI's `--level` enum option. Then, pushes these changes to the PR. If there are any SDK submodules, first those will be pushed to a PR created in the corresponding submodule repository.
